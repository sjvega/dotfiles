...
function getCurSink() {
    curSink=$(pactl list sinks | awk '/\* index:/{print $3}')
}


# Saves the sink passed by parameter's volume into a variable named `VOL_LEVEL`.
function getCurVol() {
    VOL_LEVEL=$(pactl list sinks | grep -A 15 'index: '"$1"'' | grep 'volume:' | grep -E -v 'base volume:' | awk -F : '{print $3; exit}' | grep -o -P '.{0,3}%' | sed 's/.$//' | tr -d ' ')
}
...
# `isMuted`.
function getIsMuted() {
    isMuted=$(pactl list sinks | grep -A 15 "index: $1" | awk '/muted/ {print $2; exit}')
}
